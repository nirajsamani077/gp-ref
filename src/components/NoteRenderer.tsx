...
interface Props {
  blocks:       ContentBlock[]
  searchQuery?: string
  /** Auto-scroll to the first match when opened (e.g. from command palette) */
  autoJump?:    boolean
  /** Scroll to a specific block index (from excerpt card click) */
  jumpToBlock?: number | null
}
...
// ── Exact-token highlight (yellow only, no fuzzy) ─────────────────────────────
function highlightText(text: string, query: string | undefined): React.ReactNode {
  ...
  const pattern = new RegExp(`(${tokens.map(escRe).join('|')})`, 'gi')
  ...
  while ((m = pattern.exec(text)) !== null) {
    ...
    parts.push(
      <mark key={m.index} data-match="" style={markStyle}>
        {m[0]}
      </mark>,
    )
...
// ── Resolve image src (handles GitHub Pages sub-path) ─────────────────────────
...
const navBtnStyle: React.CSSProperties = {
  ...
  borderRadius: 4, border: '1px solid #fbbf24',
...
export default function NoteRenderer({
  ...
  // ── Jump to specific block (from excerpt card click) ───────────────────────
  useEffect(() => {
    if (jumpToBlock == null || !containerRef.current) return
    const timer = setTimeout(() => {
      if (!containerRef.current) return
      const el = containerRef.current.querySelector<HTMLElement>(
        `[data-block-idx="${jumpToBlock}"]`,
      )
      if (!el) return
      el.scrollIntoView({ behavior: 'smooth', block: 'center' })
      // Activate the first mark inside that block
      const firstMark = el.querySelector<HTMLElement>('[data-match]')
      if (firstMark) {
        const idx = marksRef.current.indexOf(firstMark)
        if (idx >= 0) {
          activateMarkDirect(marksRef.current, idx)
          setMatchIndex(idx)
        }
      }
    }, 120)
    return () => clearTimeout(timer)
  }, [jumpToBlock]) // eslint-disable-line react-hooks/exhaustive-deps

  // ── Collect marks after render, handle autoJump ────────────────────────────
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (marks.length > 0 && autoJump && jumpToBlock == null) {
    ...
    return () => clearTimeout(timer)
  }, [searchQuery, blocks, autoJump, jumpToBlock]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const headings = blocks
    .map((b, i) => b.type === 'heading' && b.level === 2
      ? { text: b.text, idx: i } : null)
    .filter((h): h is { text: string; idx: number } => h !== null)

  function scrollToBlock(blockIdx: number) {
    if (!containerRef.current) return
    const el = containerRef.current.querySelector<HTMLElement>(
      ...
      {headings.length > 1 && (
        <div style={{
          ...
          {headings.map(h => (
            <button
              key={h.idx}
              onClick={() => scrollToBlock(h.idx)}
              style={{
                fontSize: 11, fontWeight: 600, padding: '3px 10px',
                borderRadius: 14, border: '1px solid #c3d9f0',
                backgroundColor: '#eef4fb', color: '#2b6cb0',
      ...
      {lightbox && (
        ...
        >
          <img
            ...
            alt={lightbox.alt}
            style={{
              maxWidth: '95vw', maxHeight: '88vh', borderRadius: 6,
...
function Block({ block, blockIdx, onImageClick, searchQuery }: BlockProps) {
  const hl = (text: string) => highlightText(text, searchQuery)
  const bx = { 'data-block-idx': blockIdx }   // shorthand for data attribute

  switch (block.type) {

    case 'heading':
      return block.level === 2 ? (
        <h2 {...bx} style={{ fontSize: 15, fontWeight: 700, color: NAVY, margin: '24px 0 8px', paddingBottom: 6, borderBottom: '2px solid #e8edf2', clear: 'both' }}>
          {hl(block.text)}
        </h2>
      ) : (
        <h3 {...bx} style={{ fontSize: 13, fontWeight: 700, color: '#2b6cb0', margin: '16px 0 6px', textTransform: 'uppercase', letterSpacing: '0.05em', clear: 'both' }}>
          {hl(block.text)}
        </h3>
      )

    case 'para':
      return <p {...bx} style={{ margin: '0 0 10px' }}>{hl(block.text)}</p>

    case 'list':
      return (
        <ul {...bx} style={{ margin: '0 0 12px', paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
    ...
    case 'table':
      return (
        <div {...bx} style={{ margin: '0 0 16px', overflowX: 'auto' }}>
          {block.caption && (
    ...
    case 'callout': {
      ...
      return (
        <div {...bx} style={{ backgroundColor: s.bg, border: `1px solid ${s.border}`, borderLeft: `4px solid ${s.border}`, borderRadius: 8, padding: '12px 16px', margin: '0 0 14px', overflow: 'hidden' }}>
          ...
          {img && (
            <img
              ...
              alt={img.alt}
              style={{
                float: imgFloat, maxWidth: img.maxWidth ?? '30%', width: img.maxWidth ?? '30%',
                borderRadius: 6, margin: imgFloat === 'right' ? '0 0 8px 12px' : '0 12px 8px 0', display: 'block',
              }}
            />
          )}
    ...
    case 'image':
      return <ImageBlock block={block} blockIdx={blockIdx} onImageClick={onImageClick} />
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {block.notes.map((n, i) => (
              <button
                ...
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, padding: '10px 16px', background: '#faf5ff', border: '1.5px solid #d6bcfa', borderRadius: 12, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}
              ...
              >
                <span style={{ fontSize: 13, fontWeight: 700, color: '#553c9a', transition: 'color 0.15s' }}
                  onMouseEnter={e => (e.currentTarget.style.color = '#fff')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#553c9a')}
                >
                  → {n.label}
                </span>
                {n.subtitle && <span style={{ fontSize: 11, color: '#718096' }}>{n.subtitle}</span>}
              </button>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {block.calculators.map((calc, i) => (
              <button
                ...
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, padding: '10px 16px', background: 'linear-gradient(135deg, #ebf8ff 0%, #e6fffa 100%)', border: '1.5px solid #90cdf4', borderRadius: 12, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}
              ...
              >
                ...
                {calc.subtitle && <span style={{ fontSize: 11, color: '#718096' }}>{calc.subtitle}</span>}
              </button>
            ))}
          </div>
        </div>
...
function ImageBlock({
  ...
  const figureStyle: React.CSSProperties = {
    margin:   isFloated ? (block.float === 'right' ? '0 0 12px 16px' : '0 16px 12px 0') : '0 0 20px',
  ...
  return (
    <figure data-block-idx={blockIdx} style={figureStyle}>
      {!errored ? (
        <div style={{ position: 'relative', borderRadius: 8, overflow: 'hidden' }}>
          ...
          {isSensitive && (
            ...
            >
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#fff', background: 'rgba(0,0,0,0.55)', padding: '6px 14px', borderRadius: 20, letterSpacing: '0.02em' }}>
